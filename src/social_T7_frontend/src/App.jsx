...
import { createActor } from '../../declarations/social_T7_backend';
import { canisterId } from '../../declarations/social_T7_backend';
import Login from './components/Login';
import CreateProfile from './components/CreateProfile';
import Dashboard from './components/Dashboard';
import './index.css';

const II_URL = 'https://identity.ic0.app/#authorize'; // ✅ always hosted II
  ...
  const initializeAuth = async () => {
    try {
      const client = await AuthClient.create({
        identityProvider: II_URL, // ✅ ensure same provider on load
      });
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      await authClient.login({
        identityProvider: II_URL, // ✅ always hosted II (no env branch)
        onSuccess: () => {

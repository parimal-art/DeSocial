  ...
  useEffect(() => {
    const loadSenders = async () => {
      ...
      for (const notification of notifications) {
        const senderPrincipal = notification.sender.toString();
        if (!senderProfiles[senderPrincipal]) {
          try {
            const res = await actor.get_user(notification.sender);
            if (res.length > 0) {
              newProfiles[senderPrincipal] = res[0];
            }
          } catch (error) {
      ...
      <div className="space-y-4">
        {notifications.length > 0 ? (
          notifications.map((notification) => {
            const sender = senderProfiles[notification.sender.toString()];
            return (
              <div
            ...
            <div className="text-gray-400 mb-4">
              ...
              >
                <path
                  ...
                  strokeWidth={1}
                  d="M15 17h5l-5 5-5-5h5v-6h0V4a1 1 0 011-1h8a1 1 0 011 1v7h0z"
                />
              </svg>
            </div>
